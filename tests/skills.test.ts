 ...
 *  - findSkill 精确命中 / 未命中
 *  - skillInvocationToken 生成 `/name` 技能调用令牌
 *  - valuePromptOf 生成「视图弹值」提示词（含指标名与数值、可选备注）
 */
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { SKILL_MODULES, findSkill, skillInvocationToken, valuePromptOf } from '../src/client/skills.ts'
...
test('skillInvocationToken：生成 `/name` 技能调用令牌', () => {
  assert.equal(skillInvocationToken(findSkill('ad-traffic')!), '/ad-traffic')
  assert.equal(skillInvocationToken(findSkill('listing')!), '/listing')
  assert.equal(skillInvocationToken(findSkill('review-insight')!), '/review-insight')
  // 令牌应单行、以 / 开头、不含冗长任务描述
  const token = skillInvocationToken(findSkill('keyword-research')!)
  assert.ok(!token.includes('\n'), '令牌应为单行')
  assert.ok(!token.includes('搜索量'), '令牌不应塞入冗长任务描述')
})
