...
var PALETTE = ["#4f7cff", "#22b573", "#f5a623", "#e5484d", "#8e5cf7", "#12a5b0", "#94a3b8"];
function buildTrendSvg(trend) {
  ...
  const x = (i) => PAD_L + i / (days.length - 1) * innerW;
...
function renderDashboard(store) {
  ...
  const trend = store.trend({ date_from: new Date(Date.now() - 29 * 864e5).toISOString().slice(0, 10) }, "day");
  ...
  const modeLabel = modeInfo.sourceMode === "rest" ? "\u5E73\u53F0 API" : modeInfo.mode === "imported" ? "\u5BFC\u5165\u6570\u636E" : "\u6F14\u793A\u6570\u636E";
  const topRows = top.length === 0 ? '<tr><td colspan="5" class="dash-empty">\u6682\u65E0\u9500\u552E\u6570\u636E</td></tr>' : top.map((p, i) => `<tr>
  ...
  const lowRows = low.length === 0 ? '<div class="dash-empty">\u5E93\u5B58\u5145\u8DB3 \u{1F389}</div>' : low.map((p) => `<div class="dash-low-item">
...
<body>
  <div class="dash-head">
    <h1>\u{1F6CD}\uFE0F \u7535\u5546\u5E97\u94FA\u7ECF\u8425\u4EEA\u8868\u76D8</h1>
    <span class="dash-badge">\u6570\u636E\u6E90\uFF1A${modeLabel}</span>
    <div class="dash-meta">
      <span>\u66F4\u65B0\u4E8E ${(/* @__PURE__ */ new Date()).toLocaleString("zh-CN", { hour12: false })}</span>
      <button class="dash-refresh" onclick="location.reload()">\u{1F504} \u5237\u65B0</button>
    </div>
  </div>
  ...
  <div class="dash-cols">
    <div class="dash-card">
      <h2>\u{1F4C8} 30 \u5929\u9500\u552E\u8D8B\u52BF\uFF08\u5CF0\u503C ${MONEY(trendSvg.max)}\uFF09</h2>
      ${trendSvg.svg}
    </div>
    <div class="dash-card">
      <h2>\u{1F9E9} \u7C7B\u76EE\u9500\u552E\u5360\u6BD4\uFF08${cats.length} \u7C7B\uFF09</h2>
      ${donutSvg}
    </div>
  </div>
  ...
  <div class="dash-foot">
    <span>ecommerce-analyst-plugin \xB7 \u6570\u636E\u53E3\u5F84\u4E0E stats_*/inventory_*/order_* \u5DE5\u5177\u4E00\u81F4\uFF08\u540C\u4E00 Store\uFF09</span>
...
function buildSnapshot(store) {
  ...
  const nameBySku = new Map(store.listProducts({ page_size: 1e3 }).items.map((p) => [p.sku, p.name]));
  for (const t of top) {
  ...
  for (const p of store.listProducts({ page_size: 1e3 }).items) {
    productCounts.set(p.category, (productCounts.get(p.category) ?? 0) + 1);
  }
  const categories = [...productCounts.entries()].map(([category, count]) => ({
    category,
    count,
  ...
  return {
    ...
    trend30: store.trend(
      { date_from: new Date(Date.now() - 29 * 24 * 60 * 60 * 1e3).toISOString().slice(0, 10) },
      "day"
    ),
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (pathname === "/ecommerce-api/import" && req.method === "POST") {
          ...
          if (parsed.monthlyReport !== void 0) {
            store.setMonthlyReport(parsed.monthlyReport);
          }
          if (parsed.monthlyPart !== void 0) {
        ...
        if (pathname === "/ecommerce-api/trend") {
          ...
          const points = store.trend(
            { date_from: new Date(Date.now() - (days - 1) * 24 * 60 * 60 * 1e3).toISOString().slice(0, 10) },
            "day"
          );
        ...
        if (pathname === "/ecommerce-api/products") {
          const category = query.get("category") || void 0;
          sendJson(res, 200, {
            ok: true,
...
function buildActions(store) {
  ...
  for (const o of t.overdues) {
    actions.push({
      ...
      detail: `${o.buyer} \xB7 \xA5${o.amount.toFixed(2)} \xB7 ${o.created_at.slice(0, 10)}`,
      urgent: true
    });
  }
  for (const o of t.shipments) {
    actions.push({
      ...
      detail: `${o.buyer} \xB7 ${o.product_name} \xD7${o.quantity} \xB7 \xA5${o.amount.toFixed(2)}`,
      urgent: false,
  ...
  for (const p of store.lowStock()) {
    actions.push({
      ...
      kind: "low_stock",
      title: `\u4F4E\u5E93\u5B58 ${p.name}`,
      detail: `${p.category} \xB7 \u5E93\u5B58 ${p.stock}`,
      urgent: p.stock === 0
    });
  }
...
function buildBrief(store) {
  const o = store.overview();
  const today = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const nameBySku = new Map(
    store.listProducts({ page_size: 1e3 }).items.map((p) => [p.sku, p.name])
  );
  ...
  lines.push("## \u56DB\u3001\u4ECA\u65E5\u5F85\u529E");
  lines.push(
    `- \u26A0\uFE0F \u903E\u671F\u8BA2\u5355 **${t.overdues.length}** \u7B14 \uFF5C \u{1F4E6} \u5F85\u53D1\u8D27 **${t.shipments.length}** \u7B14 \uFF5C \u26A0\uFE0F \u4F4E\u5E93\u5B58 **${t.lowStockCount}** \u4EF6`
  );
  lines.push("");
