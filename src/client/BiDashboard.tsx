/**
 * ecommerce-analyst-plugin — BI 数据看板（客户端）
 *
...
import { formatMoney, type ShopSnapshot } from './data.ts'
...
function BiKpiGrid(props: { snapshot: ShopSnapshot; onValue: (text: string, okText?: string) => void }): React.ReactElement {
...
function BiTrendChart(props: { points: Array<{ date: string; revenue: number; orders: number }>; onValue: (text: string, okText?: string) => void }): React.ReactElement {
  ...
  return (
    <div
      ...
      onClick={() => props.onValue(valuePromptOf('近 30 天销售趋势', `最新 ${last.date} 销售额 ${formatMoney(last.revenue)} · ${last.orders} 单（共 ${pts.length} 天）`))}
    >
...
function BarRow(props: { item: BarRowItem; max: number; ratio: number; onClick?: () => void }): React.ReactElement {
  const { item, ratio } = props
  return (
    <div className={'esd-bi-bar-row' + (props.onClick !== undefined ? ' esd-clickable' : '')} key={item.key} onClick={props.onClick}>
...
function BiCategoryBars(props: { categories: Array<{ category: string; revenue: number; ratio: number }>; onValue: (text: string, okText?: string) => void }): React.ReactElement {
    ...
    <div className="esd-bi-bar">
      {items.map((it, i) => (
        <BarRow
          ...
          max={100}
          ratio={it.value}
          onClick={() => props.onValue(valuePromptOf('类目销售占比', `${cats[i].category} · ${formatMoney(cats[i].revenue)} · 占比 ${cats[i].ratio.toFixed(1)}%`))}
        />
      ))}
...
function BiTopBar(props: { top: Array<{ sku: string; name: string; revenue: number }>; onValue: (text: string, okText?: string) => void }): React.ReactElement {
    ...
    <div className="esd-bi-bar">
      {rows.map((it, i) => (
        <BarRow
          ...
          onClick={() => props.onValue(valuePromptOf('TOP 商品销售额', `${items[i].name}（${items[i].sku}）· ${formatMoney(items[i].revenue)}`))}
        />
      ))}
...
export function BiDashboardSection(props: { snapshot: ShopSnapshot; onValue: (text: string, okText?: string) => void }): React.ReactElement {
    ...
    <div className="esd-bi">
      ...
      <div className="esd-bi-grid">
        <div className="esd-bi-card">
          ...
          <BiTrendChart points={s.trend30} onValue={props.onValue} />
        </div>
        <div className="esd-bi-card">
          ...
          <div className="esd-bi-stock">
            ...
            {s.lowStock.slice(0, 6).map((p) => (
              <div
                ...
                onClick={() => props.onValue(valuePromptOf('低库存商品', `${p.name}（${p.sku}）· ${p.category} · 库存 ${p.stock} 件`))}
              >
      ...
      <div className="esd-bi-grid">
        <div className="esd-bi-card">
          ...
          <BiCategoryBars categories={s.categories} onValue={props.onValue} />
        </div>
        <div className="esd-bi-card">
          <div className="esd-bi-card-title">TOP 5 商品销售额排行</div>
          <BiTopBar top={s.top} onValue={props.onValue} />
        </div>
      </div>
    </div>
  )
