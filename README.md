...
同一份 8 月数据此前在不同模块给出了互相矛盾的数字，已定位并修正（统一为「同源」口径）：

| 位置 | 修正前 | 修正后 | 说明 |
|---|---|---|---|
| 数据对比 · 趋势 KPI 净销额 | ¥336.5 万 | **¥449.2 万** | 原取「利润表」并自算 销售收入−退款，与同屏明细表差 112.7 万 |
| 数据对比 · 趋势 KPI 费比 | 16.20% | **12.20%** | 分子取利润表、分母却用利润表派生的净销额 → 跨源混用 |
| 销售概览 · 洞察条费比 | 9.46%（走链接分支时） | **12.19%** | 分子是链接推广费、分母却用销售额 |

- **根因一（跨源混用）**：`buildMonthTrendPoint` 优先取「利润表」，而「对比明细」取「排名表」，同一个「净销额」在同一屏出现两个数。现统一为**排名表**（层级优先 平台链接 → 系统货品 → 系统规格，随所选层级切换），并在趋势指标条上标注「取数层级：平台链接（与下方明细表同源）」。利润表仅用于经销排行与概览条的店铺分支 —— 那里费比 = 推广费 ÷ 销售收入（12.72%），与该表 ad_ratio 口径一致。
- **根因二（字段不存在导致退款被吞）**：原代码用「销售额 − 退款」反算净销额，但 `WeeklyProductRow`（系统货品行）**根本没有 `refundAmount` 字段**（只有链接 / 规格 / 店铺层级有），运行时取到 `undefined` → 退款按 0 处理，货品 / 规格层级净销额虚高。现**直接取表内「净销售额」列**求和，不再反算。
- **费比口径**：分母一律与分子同源 —— 排名表口径 = 推广投放费用 ÷ **净销售额**；利润表口径 = 推广运营费用 ÷ **销售收入**。已在 `compare.ts` 与面板两处修正。
- 回归单测：新增「趋势 KPI 净销额必须等于同屏明细表总计」「货品层级不得用 销售额−退款 反算」两条，锁定上述问题。
