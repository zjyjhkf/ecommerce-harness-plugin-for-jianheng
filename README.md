...
## v0.4.5 清除彻底治理 + 对比柱线合一 + 多月份面板规则（增量）

- **🧹 清除彻底治理（根治「清除后销售概览图表/排行清不掉」）**：根因是各视图空态只重写表格，**echarts canvas 不会自动清除**，旧月份柱形/折线残留画布。现在 `renderView` 入口统一判定「当前周期无数据」→ 该视图的 KPI、全部明细表、**每一张图表画布**逐一无条件重置为「暂无数据 · 请先导入」占位；7 个视图（销售/产品/明细/SKU/推广/退款/复盘）的图表清单在 `VIEW_EMPTY_TARGETS` 集中登记，任何导入/清除路径共用同一出口。测试用真实数据→清空→重渲染两轮回验（vm 执行页面原脚本）。
- **📊 数据对比布局美化：柱+折线合并为一图**（对齐「多月份.html」形态）：废弃旧版上下两张各占 100% 宽的图；现在**一张组合图**——柱=所选指标每月一根（**每月不同色**、最近导入月高亮橙、柱顶数值），虚线折线=**环比增速%**（右轴，点色随月），tooltip 同图联动。趋势 KPI 五卡一行（`.kpi-grid.five`，窄屏降两列）。
- **🗓 多月份面板规则**：连续导入 A、B、C 三个月后——**除数据对比外的所有面板只显示 C（最近导入的那个月）**；数据对比面板显示 **A/B/C 全部月份**对比（两月就只对比两月）。KPI 大字/高亮柱锚定**最近导入月**而非字典序最大月（先导 8 月、再补导 7 月时大字=7 月）；柱/折线 x 轴仍按月份自然升序。
- 面板资源版本号 `data-center?v=20260914-r22`。

---

## v0.4.4 对比按键交互修复 + 数据对比跳转入口（增量）

- **🔧 对比视图按键无反应根因修复**：层级/指标切换函数调用了一个**不存在的 `compareRender()`**（v0.4.2 遗留笔误）——点击即 `ReferenceError`，视图永不更新，表现为「按键模块无法交互和跳转」。现改为「本地缓存即时重绘 `compareFrame()` + 服务端拉取新层级/指标 `compareRefresh()`」，两个入口均已修复并有测试锁死（禁止再出现 `compareRender()` 调用）。
- **➕ 销售概览新增「数据对比」跳转卡**（模块速览末位）：出现/隐藏与左侧「数据对比」菜单**同一判定源**（`showModule`）——连续导入两月才出现、点击直达对比视图、清除后同步消失。
- **🧹 工作台清除按钮 tooltip 修正**：不再宣称「自动下载备份快照」（与 v0.4.2 起的纯清除语义一致，杜绝误解）。
- 面板资源版本号 `data-center?v=20260914-r21`。
...
## v0.4.3 数据对比多月份趋势 + 销售概览核心速览 + 导出修复（增量）

- **📊 数据对比 = 多月份趋势**（对齐「电商数据中台_多月份」月度复盘形态）：
  - Store 新增**按月归档**（`monthlyHistory`，最多 12 期，随持久化保存/恢复，`clearAllData` 一并清空）。
  - 顶部 KPI：**大字 = 最新月（A），小字 = 上一月（B）+ 环比**；**毛利与费比拆成两张独立卡**（不再合并），另含净销额 / 产品规格数 / 推广费。
  - 指标按键（净销额 / 产品规格数 / 推广费 / 毛利 / 费比）一键切换两张图：**按月柱状图（每月不同色、最新月高亮橙，直观区分对比）** + **随月份推进的折线走势图**。
  - `/ecommerce-api/compare` 负载新增 `trend` / `trendMetrics` / `showModule`；实体「上期 vs 本期」明细排行保留为对比核心。
  - 对比指标目录**精简删非核心按键**：正向销售收入 / 仓库物流费 / 毛利率 / 客单价 / 浏览量 等移除，仅留销售额/净销额/毛利/推广费/退款率（利润表再加费比）。
- **✅ 「连续导入两月 → 数据对比必现」彻底修复**：旧版数据变化后只清缓存、等用户点进对比视图才拉取——但此时菜单仍隐藏、用户点不进（鸡生蛋死锁）。现数据一变即 `compareRefresh()`，按服务端 `showModule` 决定显隐：**满两月必现，清除后必隐，重导再现**（端到端用真实 7 月 + 8 月源文件验证）。
- **🧮 清除彻底化**：无月报（含清除后）也走一遍月份标签 / 平台筛选 / 侧栏统计 / 顶部日期刷新，清除后顶部日期回到「待导入月度数据」，不留旧月残影。
- **📈 销售概览核心速览**（明细排行上方新增三柱状）：退款核心（逐店退款额柱 + 退款率折线双轴）、品类销售（货品按分类聚合柱）、推广费用（逐店推广费柱）；点击任一柱形 → 会话分析联动。
- **📤 CSV 导出修复（导出空表问题）**：根因是导出只取商品/订单，而用户数据主体是复盘 Excel。现在
  `scope=all` 会一并导出**全部月度归档 + 周复盘**（新增 `scope=reports`）；JSON 导出含 `monthlyHistory`；
  完全无数据时返回 **400 `EXPORT_EMPTY`** 并提示先导入，不再产出「只有表头的空 CSV」。
- 面板资源版本号 `data-center?v=20260914-r20`。

---

## v0.4.2 清除重置改「纯清除」+ 数据对比按需出现（增量）

- **🧹 清除重置 = 单一职责**：确认 → 清空全部已导入数据（商品/订单/月周复盘/**对比归档**）→ 面板立即刷新为空白。
  不再产生/下载任何备份文件（v0.4.1 的自动快照下载在部分 WebView 中抛错会阻断刷新通知，导致"点了没清"——已根治；iframe 侧另有 4s 轮询双保险）。
- **空库 = 面板全空白**：`POST /ecommerce-api/clear-data` 清除后各视图均为"未导入"空态。
- **「数据对比」只在连续导入两期后出现**：单期数据或清除后，对比菜单项整体隐藏；正停留在对比视图会自动回到默认视图。
- 面板资源版本号 `data-center?v=20260911-r19`（强制绕过 WebView 旧缓存）。

---

## v0.4.1 精确性修订（增量）

- **面板右上角新增「清除重置」按钮（🧹）**：二次确认后清空当前全部已导入数据（商品/订单/月度/周度复盘及归档）；清空前自动下载全量备份快照（与导入备份同格式，导回即完整恢复）。API：`POST /ecommerce-api/clear-data`。
- **数值结转修复**（7月/8月真实月度数据逐分验证驱动）：`platformLinks` 无身份占位行（导出残留）的数字不再随剔行丢弃——可加指标结转为一条明确标注的汇总行（比率字段置 0，不参与任何排行/最值），面板合计与源表逐分一致。
- **CI 化精确性校验脚本**：`node --import tsx scripts/verify-accuracy.mjs verify [目录...]` —— 走生产导入链路聚合出面板数据，再用 xlsx 独立重算源表合计逐指标对比，任何不一致退出码非 0。
...
- **数据对比（多月份趋势）**：**连续导入两个及以上月份后，「数据对比」侧边栏模块必定出现**（按月归档，最多保留 12 个月）；顶部 KPI **大字 = 最新月、小字 = 上一月**，毛利与费比拆分为独立卡片；指标按键切换 **净销额 / 产品规格数 / 推广费 / 毛利 / 费比**，每个指标都有「每月不同色」的对比柱状图与随月份推进的折线走势图。点「清除」后归档全部归零，**该模块立即隐藏**（隔离），重新连导两月后再次出现。
...
```sh
# 单元测试（160 项：状态机、统计口径、库存预警、持久化、工具注册、周期隔离、导出接口、入口冒烟、UI 完整性、数据对比引擎）
