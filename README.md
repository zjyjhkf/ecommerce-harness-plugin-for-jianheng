...
| 产品概览 / 商品明细 / SKU 规格 KPI | **商品排名口径**（平台商品排名表） | 578.9 万（KPI 副标题已写明「商品排名口径」） |

- **两套口径必须分开标注**：利润表是发货口径的财务数（428.4 万），平台商品排名表是商品口径（578.9 万，含未发货 / 跨期）。同一屏里只许**头寸**（概览条、洞察条、趋势 KPI、月复盘合计）走利润表；商品层模块允许用排名表，但 KPI 副标题必须写明来源口径，避免出现「净销额 449 万 > 销售额 428 万」这类跨口径比较。
...
- **排名表两列自相矛盾**：三张排名表（货品 / 规格 / 链接）的「净销售额」列彼此一致，但都比重算值 `销售额 − 退款金额` 低 2,620~3,454 元（约 0.06%）。按用户口径，链接层与规格层一律改用 `销售额 − 退款金额`（`enforceNetSalesRule`）；货品层源表没有退款列（`WeeklyProductRow` 无 `refundAmount`），保留表内「净销售额」列，与链接 / 规格层相差约 2,629 元，属源表自身差异。
