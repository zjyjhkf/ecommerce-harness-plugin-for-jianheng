...
>
> 商品/订单数据**完全由「数据导入」或「平台 API」决定**，不提供手动增删改查工具，避免与导入数据冲突。
...
侧边栏新增「新品追踪」，由**当期已导入**的「系统货品表 + 系统规格表」现算新品与老品的对比，无任何内置或写死数据。

- **新品判定**（面板顶部标明实际用了哪一条）：
  1. **上市月份** —— 月度表「分类」列是 `26年7月` 这类上市月份标签时，取标签 = 本期报表月份的那批货品 / 规格；
  2. **首次上榜** —— 「分类」列是商品分组（`货品周期` / `热销期` / `球类` …）而无月份标签时，回退为「上一期未出现、本期首次出现」；
  3. 两条都不成立（未导入 / 缺上一期 / 确无新品）→ 面板直说原因，不显示空图。
- **口径**（面板「判定口径」条逐条写明）：
  - 跨期比对按**货品名** —— 相邻两期的货品编号是两套编码体系（同一个「空气压力波」7 月是 `Z-kqylb`、8 月是 `T-KQB-001`），按编号比会把真新品虚增数倍；
  - 单期内划分新品 / 老品按**货品编号** —— 真实数据存在同名不同编号的货品，按名字划分会误判；
  - 毛利率分母为**净销售额**（与表内「毛利率」列一致）；汇总比率一律**按销售额加权**。
- **展示**：判定口径条 → 两排 KPI（新品数 / 新品销售额与占全店 / 毛利率对比 / 退款率对比 / 销售件数 / 销售成本与成本率 / 退款三段拆解 / TOP1 集中度）→ 双页签（**新品货品** / **新品规格明细**）→ 7 张图（TOP10 货品、新品构成饼、新品 vs 老品关键指标、退款阶段拆解、TOP10 规格、TOP1 规格构成、规格数分布）→ 两张明细表。
- **接口**：`GET /ecommerce-api/new-products`（服务端现算，返回的 `basis` / `basisLabel` 标明判据，`prevSide` 为上一期新品概览）。该负载同时供「数据对比 · 新品对比」使用。

### 销售额 / 净销售额 / 费比口径（用户指定，全站唯一口径）

**两套数据源、两条取数规则：**

```
① 利润表（发货口径财务数，用于全店头寸）
   销售额       = 正向销售额 = 利润表「销售收入」     （8 月 428.4 万）
   净销售额     = 销售额 − 退款                        （8 月 428.4 − 91.9 = 336.5 万）
   费比         = 运营推广费用 ÷ 销售收入              （8 月 12.72%）

② 排名表（平台商品排名导出，用于商品层：货品 / 链接 / SKU）
   销售额               = 源表「净销售订单 / 净销售额」列（8 月 449.2 万）
   订单销售额（含退款） = 源表「销售订单 / 销售额」列      （8 月 578.9 万）
   退款金额             = 源表「合计退款 / 退款金额」列    （8 月 129.3 万）
   毛利率 = 毛利额 ÷ 销售额（= 源表「毛利率」列自己的算法，逐行核对一致）
   费比   = 推广投放费用 ÷ 销售额
```

| 位置 | 口径 | 8 月数字 |
|---|---|---|
| 概览条 · 销售额 / 退款金额 / 净销售额 | 利润表（财务口径） | 428.4 万 / 91.9 万 / **336.5 万** |
| 经营洞察条 | 同上 | 同上 |
| 月复盘 · 全店经营指标合计 | 同上 | 同上（「销售收入（正向）」单行，不并列两次） |
| 数据对比 · 趋势 KPI | 同上（`source='storeProfit'`，标注「财务口径·利润表」） | 净销额 336.5 万 |
| 产品概览 · 货品销售额 | 排名表「净销售额」列 | **449.2 万**（KPI 副标题写明来源列） |
| 产品概览 · 货品订单销售额（含退款） | 排名表「销售额」列 | 578.9 万 |
| 商品明细 · 商品销售额 / SKU 规格 · SKU 销售额 | 排名表「净销售额」列 | 449.2 万 |

- **为什么排名表的「销售额」取「净销售额」列**：源表那一列才与用户口径的销售额一致 —— 8 月产品表「销售额」列 618 行合计 5,788,518.93（含退款的订单额），「净销售额」列合计 4,492,466.32；链接表与规格表的净销售额列同为 449.2 万（三表彼此一致）。按用户指定，**「货品销售额」显示 449.2 万**，含退款的 578.9 万一律改称「订单销售额（含退款）」，避免 5 字头数字被当成全店销售额。
- **两套口径必须分开标注**：利润表是发货口径的财务数（428.4 万），排名表是商品口径（449.2 万）。同一屏里只许**头寸**（概览条、洞察条、趋势 KPI、月复盘合计）走利润表；商品层模块走排名表，但每张 KPI 的副标题都写明来源列。
- **源表两列自身的 0.06% 偏差不再掩饰**：`销售额 − 退款金额` 比表内「净销售额」列高 2,620~3,454 元（8 月 2,629 元）。用户口径以表内列为准，因此 `applyNetSalesRule` 只在表内列缺失 / 为 0 时用相减兜底，正常情况原样保留（该函数不再覆盖链接层与规格层）。
- **`正向销售收入` 列不被采用**：源表「利润表 · 正向销售收入(不含特殊单)」实测公式写错（写成 销售收入 + 退款，应为 −），解析层按用户口径令 `positiveSales := sales`，原值保留在 `positiveSalesRaw`，仅在面板顶部提示条与 `ecommerce_data_report` 工具输出里提示核对 Excel。
- **费比分母与分子同源**：利润表口径 = 推广运营费用 ÷ 销售收入（8 月 12.72%，与该表 `ad_ratio` 一致）；回退排名表时 = 推广投放费用 ÷ 净销售额。
- 回归单测：`tests/compare-trend.test.ts` 锁定「净销额恒等于 销售额 − 退款，绝不出现净销额 > 销售额」「有利润表时趋势必走财务口径」「无利润表时才按层级回退并与明细表同源」。

### 数据对比：多视图重排 + 无效阈值筛选 + 新品对比

- **对比明细列改造**（参考《电商数据中台_多月份.html》的对比表列结构）：表头由笼统的「上期 / 本期」改为**真实月份 + 指标**（如「7月销售额 / 8月销售额」），并新增 **品类**、**编号** 身份列、**毛利率(上期→本期)** 对比列（单元格形如「43.1% → 43.2% ↑」）与 **状态** 列（新上榜 / 已退出 / 上升 / 下降 / 持平）。
  - **动态剔除「数据全部为空」的列**：渲染时逐列检测，整列无有效值即整列隐藏并在徽标里写明（例：链接层级源表没有「分类」列 → 自动隐藏「品类」；比率类指标没有「变化率」→ 自动隐藏）。列样式用列名 class 而非 `nth-child`，避免剔除后列位置错位。
  - 毛利率 = Σ毛利额 ÷ Σ净销售额，两列均来自导入 Excel；**未采用参考表的「净利率(上期→本期)」** —— 其净利额含平台服务费等分摊项，用现有列无法精确复现（实测最接近的组合为 59.74%，参考值 57.43%），不编造对不上的数字。
- **新品对比**：（新增）：连续导入两期后出现的「数据对比」模块内新增一个「新品对比」区块，**数据直接取自上面的「新品追踪」负载**（同一份 `/ecommerce-api/new-products`，不另算新品，两处口径不可能分叉）：
  - 上期 vs 本期新品关键指标分组柱（占全店销售额 / 新品毛利率 / 新品退款率 / TOP1 集中度）+ 本期新品 TOP10 销售额；
  - 徽标写明「新品数 上期 → 本期 · 规格 上期 → 本期」；
  - **只展示有效数据**：复用对比模块自身的无效阈值（与下方「对比明细」同一套），说明条写明剔除了几个、阈值是什么；
  - 明细表为「本期新品（仅有效数据）」，含规格数 / 本期销售额 / 占新品 / 毛利率 / 退款率 / 退货比例；
  - 上一期的新品概览同样由新品追踪的同一套判据算出（`prevSide`）；**两期判据不一致时说明条会显式提示**（例：上期「分类上市月份」vs 本期「首次上榜」），避免拿不同口径的数量直接对比。
- **布局**：由「一张柱状图占满横向」改为多视图混排 —— 月度对比（柱=每月不同色 + 线=环比增速）｜各月结构占比 → 层级/指标按键 + KPI → 增长 TOP10｜下滑 TOP10｜增减额×增减率四象限 → 名次变动｜本期构成帕累托 → 对比明细（全宽）。
- **无效阈值剔除**：按「变动中位数 8% / 本期总量 0.02% / 两期较大值 2%」三者取大，剔除双零、变动过小、体量过小的对象；**图表与表格共用同一有效集**，可切换「仅显示有效数据 / 显示全部」，阈值与剔除数在工具条上写明。
- **榜单页签**：全部 / 增长榜 / 下滑榜 / 新增 / 退出，计数与筛选实时联动。
- **可读性**：明细表固定视口 600px + 表头吸顶 + 200 行渲染上限；全站图表字号 ≥ 11px，长名称用省略号而非挤压。

> ⚠️ **安装提醒（重要）**：`npm install` 只会更新你执行命令的那个目录，**不会更新 dsh 正在加载的 profile**。dsh 桌面 / 网页端实际加载的是 `~/.dsh/profiles/web/node_modules/ecommerce-analyst-plugin`，请用 **`dsh plugin --profile web add ecommerce-analyst-plugin@latest`** 升级并**重启 dsh 进程**（只刷新页面不会重新加载插件）。

---

## 会话激活门控：默认静默 / 显式启用

**为什么需要门控**：插件若一经装载就把「今日待办」注入**所有** dsh 会话的系统提示、并全量注册 30+ 业务工具，用户哪怕只说「你好」，模型也可能自行调用工具、凭空谈论并不存在的店铺数据（幻觉）。

**默认 `silent`（静默激活）**：不注册任何业务工具、不注入任何系统提示段落 —— 对 dsh 会话**零影响**。仅保留：

- 技能目录（`skills/*` 7 个分析技能，`/name` 显式调用入口）；
- 侧边栏「电商数据中台」与 `/ecommerce-api`（纯 UI，不进模型上下文）。

**数据来源**：包内**不含任何演示种子数据**，运行时以**空库**启动；数据只能来自 Excel / 备份导入或 rest 平台 API。
...
- 启用后全量注册业务工具并注入今日待办（空库时显示「今日无待办」，不会编造数据）。

> 测试基建：测试专用 fixture 位于 `tests/fixtures/seed.json`（经 `tests/seed-fixture.ts` 注入），生产代码与 npm 包均不携带。
...
> 本包已发布到 npm 官方 registry，包名 `ecommerce-analyst-plugin`，
...
> 默认以**空库**启动（无任何内置示例数据）。启用方式见「会话激活门控」，取数方式有两种：**导入 Excel 复盘**（侧边栏「电商数据中台」→ 导入），或按「对接真实电商平台」章节切到 rest 平台 API。
...
| 🔄 数据源切换 | `ecommerce_set_mode` | 导入数据 / 平台 API 显式切换（无演示数据模式） |
| 🗂️ 取数唯一入口 | `ecommerce_data_report` | 模型回答任何数据问题前的统一取数口（总览 / 月份清单 / 店铺利润 / 系统货品 / 平台链接 / 系统规格 / 周期对比），与面板同源同口径 |

## 月度/周度复盘导入 + 数据中台

插件内置「电商数据中台」，把电商平台导出的**月度表（4 份 Excel）** 与**周度数据（3 份 Excel）** 解析成经营分析面板：

- **月度复盘（30 天「月复盘」）**：一次性导入 4 份 Excel —— 三份「商品排名导出」（平台货品/系统货品/系统规格）+ 一份「利润表」。
- **周复盘（7 天「周复盘」）**：导入 3 份「商品排名导出」（平台货品/系统货品/系统规格）。
- **周期严格隔离**：7 天与 30 天数据分开存放与展示。**只导入 7 日周数据时，30 天面板保持为空**（不插入数据就不显示对应面板）；反之亦然。
- **数据对比（多月份趋势 + 多视图）**：**连续导入两个及以上月份后，「数据对比」侧边栏模块必定出现**（按月归档，最多保留 12 个月），连续导入两期时自动归档上一期；顶部 KPI **大字 = 最新月、小字 = 上一月**，指标按键切换 **净销额 / 产品规格数 / 推广费 / 毛利 / 费比**。视图为多视图混排：月度对比（柱=每月不同色 + 线=环比增速）｜各月结构占比 → 增长 TOP10｜下滑 TOP10｜增减额×增减率四象限 → 名次变动｜本期构成帕累托 → 对比明细；明细按**无效阈值**（变动中位数 8% / 本期总量 0.02% / 两期较大值 2% 取大）剔除双零与体量过小的对象，并可切换「仅显示有效数据 / 显示全部」与「全部 / 增长榜 / 下滑榜 / 新增 / 退出」榜单页签。点「清除」后归档全部归零，**该模块立即隐藏**（隔离），重新连导两月后再次出现。
- **新品追踪**：侧边栏「新品追踪」模块由当期导入的系统货品表 + 系统规格表现算新品与老品的对比（判定口径与展示内容详见本文档开头的「当前版本 v0.4.9」）。
- **数据导出**：CSV/JSON 导出现在覆盖**商品/订单 + 全部月度归档与周复盘报表**（`/ecommerce-api/export?scope=products|orders|reports|all`）；无任何数据时返回明确的「请先导入」错误，不再产出空 CSV。
...
> 打开方式：右侧底部「数据查看」圆形入口（`sidebar.footer.action`）点击展开面板；面板以 `conversation.view` 标签页形态呈现，`shell.overlay` 兜底。面板只承载「电商数据中台」。
...
默认以**空库**启动，数据来自 Excel 导入。接入真实电商平台 API 时：
...
- **降级机制**：API 不可用时回退到已导入数据（空库则明确提示「暂无数据，请先导入」），不编造数据

## 开发与测试

```sh
# 单元测试（168 项：状态机、统计口径、库存预警、持久化、工具注册、周期隔离、导出接口、入口冒烟、UI 完整性、数据对比引擎、新品追踪口径）
...
├── new-products.ts       # 新品追踪聚合（新品判定 + 新品/老品指标现算，纯函数）
...
│   ├── mock.ts           # 本地空适配器（默认，只读；不带任何示例数据，仅测试可注入 fixture）
...
└── tools/
    ...
    ├── data-report.ts    # 统一取数工具（ecommerce_data_report）
    └── json.ts           # 输出类型适配
tests/                    # 168 项单元测试
